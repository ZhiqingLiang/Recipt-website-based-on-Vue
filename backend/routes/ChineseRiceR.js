...
router.post('/addChineseR',async(req,res)=>{
    ...
    try{
        ...
        const duplicate = await ChineseRice.findOne({name:req.body.name})
        if(duplicate){
...
router.get('/getChineseR',async(req,res)=>{
    try{
        console.log('Starting to fetch recipes'); // Debugging information
        const recipes = await ChineseRice.find();
...
// Delete recipe
router.delete('/delChineseR/:id',async(req,res)=>{
    try{
       // Get the id in the request parameters
        ...
        const result = await ChineseRice.findOneAndDelete(ObjectId);//Delete the contents of the database according to id
        if (!result) {
...
router.get('/getID/:id',async(req,res)=>{
    try{
        ...
        const ObjectId = new mongoose.Types.ObjectId(id);
        const recipes = await ChineseRice.findById(ObjectId);
        // console.log(recipes);
        if(!recipes){
