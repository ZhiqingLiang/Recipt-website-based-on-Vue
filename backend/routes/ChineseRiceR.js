...
import {ChinsesRice} from '../models/ChinsesRice.js'
...
router.get('/getChineseR',async(req,res)=>{
    try{
        ...
        const receipts = await ChinsesRice.find();
...
router.delete('/delChineseR/:id',async(req,res)=>{
    try{
       // Get the id in the request parameters
        ...
        const result = await ChinsesRice.findOneAndDelete(ObjectId);//Delete the contents of the database according to id
        if (!result) {
...
router.get('/getID/:id',async(req,res)=>{
    try{
        ...
        const ObjectId = new mongoose.Types.ObjectId(id);
        const receipts = await ChinsesRice.findById(ObjectId);
