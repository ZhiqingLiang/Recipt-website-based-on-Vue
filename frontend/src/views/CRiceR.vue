...
<!-- This 'template' is for Chinese Rice Recipe -->
<template>
  <div class="container">
    ...
    <div class="recipeBox" v-for="box in boxes" :key="box._id">
      <ul class="boxlist" >
        <li class="box" >
          <a href="./CRiceTemplate"  @click.prevent="navigate(box._id)">
              <div class="image">
...
export default {
  created(){
      console.log('CRice Component created');
      this.fetchData();
    },
  data(){
    return{
  ...
  methods:{
    async handleFormSubmit(form){
      ...
      // this.boxes.push(form);
        const res = await axios.post('http://localhost:3000/api/ChineseRice/addChineseR',form);
        if(res.data.success){
    ...
    async fetchData(){
      ...
      // Send request to back end
        const res = await axios.get('http://localhost:3000/api/ChineseRice/getChineseR');
    ...
    async del(id){
      ...
      try{
        const res = await axios.delete(`http://localhost:3000/api/ChineseRice/delChineseR/${id}`)
    ...
    navigate(id){
      this.$router.push({name:'CRiceTemplate',params:{id:id}});
      console.log(`Navigating to CRiceTemplate with id: ${id}`);
    }
  }
}
</script>
...
.container{
  background-image: url('@/assets/img/ricebg.jpg');
